...
const LS_KEY = "jhse_vocab_users_v1";
...
const RANKS = [
  ...
  { key: "bronze",    name: "ブロンズ",  needOk: 100,  css: "rank-bronze" },
  { key: "silver",    name: "シルバー",  needOk: 200,  css: "rank-silver" },
  { key: "gold",      name: "ゴールド",  needOk: 350,  css: "rank-gold" },
  { key: "platinum",  name: "プラチナ",  needOk: 500,  css: "rank-platinum" },
  { key: "diamond",   name: "ダイヤモンド", needOk: 1000, css: "rank-diamond" },
  { key: "master",    name: "マスター",  needOk: 2000, css: "rank-master" }
];
...
function ensureUser(name){
  if (!users[name]) {
    users[name] = {
      totalAns: 0,
      totalOk: 0,
      miss: {} // enLower -> {en, ja, missCount, lastMissAt}
    };
    saveUsers();
  }
}
...
function calcRank(totalOk){
  ...
  let remain = 0;
  if (!isMax) remain = Math.max(0, next.needOk - totalOk);

  // 進捗バー：現在ランクの区間内で 0-100
  ...
  return { idx, cur, next, isMax, remain, pct };
}
...
function updateRankUI(){
  ...
  const u = getUser();
  const { cur, next, isMax, remain, pct } = calcRank(u.totalOk);
...
function onAnswerMCQ(choice, btnEl){
  ...
  if (isCorrect) {
    ...
    btnEl.classList.add("pulseOk");
  } else {
    btnEl.classList.add("wrong");
    btnEl.classList.add("pulseNg");
...
function startNormalQuiz(){
  ...
  if (!lv || !lv.words || lv.words.length < 10) {
    alert("単語数が少なすぎます。words.json の words を増やしてください。");
    return;
  }

  currentQuiz = buildQuiz(lv.words, mode);
  show(els.quiz);
  renderQuestion();
}
